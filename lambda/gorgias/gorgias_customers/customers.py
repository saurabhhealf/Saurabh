"""
customers.py — Gorgias /customers daily fetch → S3 JSONL

Goals:
- Daily incremental fetch: fetch pages until updated_datetime < target_date
- No crash-loop on HTTP 429: backoff + checkpoint + graceful exit
- Prevent parallelism: DDB lease lock
"""
...
def handler(event, context):
    ...
    # --- Date Window Logic ---
    # Default: look back 2 days if not specified
    ...
    try:
        while processed < PAGES_PER_INVOCATION and _time_left_ok(context):
            ...
            try:
                ...
                # Allow graceful exit on persistent failures
                msg = str(e)
            ...
            for item in items:
                ...
                if upd_dt >= start_of_window:
                    ...
                    # Since sorted desc, once we hit an old item, all subsequent items are old
                    break
            ...
            # Stop condition 1: Data is too old
        ...
        # Loop finished (time limit or page limit) -> Checkpoint and pause
