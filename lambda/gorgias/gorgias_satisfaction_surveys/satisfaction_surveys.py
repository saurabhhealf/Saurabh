"""
satisfaction_surveys.py — Gorgias /satisfaction-surveys extractor (Backfill + Daily) → S3 JSONL
...
def _ddb_checkpoint(job_start_id: str, status: str, page: int, cursor: Optional[str], note: str = "", last_error: str = "") -> None:
    ...
    # 2. Add all SET attributes FIRST (Cursor, Note, Error)
    if cursor:
        expr += ", #cursor=:c"
        names["#cursor"] = "cursor"
        vals[":c"] = cursor
    ...
    # CRITICAL FIX: This must be appended AFTER all SET clauses
    if not cursor:
        expr += " REMOVE #cursor"
        names["#cursor"] = "cursor"
...
def _fetch_page(session: requests.Session, cursor: Optional[str]) -> Tuple[List[Dict[str, Any]], Optional[str], Dict[str, str]]:
    ...
    # Gorgias Satisfaction Surveys Sort Order
    # Note: 'created_datetime' is standard for surveys
    params = {
        "limit": PAGE_SIZE,
    ...
    if isinstance(j, dict):
        for k in ["data", "items", "results", STREAM_NAME, "satisfaction_surveys"]:
...
def handler(event, context):
    ...
    # Daily logic: lookback 2 days if target_date exists but not explicit
    # Backfill logic: target_date is None, so fetch everything
    is_daily = target_date_str is not None
    
    if is_daily:
        try:
            # Surveys use created_datetime usually
    ...
    try:
        while processed < PAGES_PER_INVOCATION and _time_left_ok(context):
            ...
            if is_daily:
                for item in items:
                    # Surveys usually rely on created_datetime
                    cdt = item.get("created_datetime")
                    if not cdt: continue
                    try:
                        dt_obj = datetime.fromisoformat(cdt.replace("Z", "+00:00"))
                    except:
                        continue
                    
                    if dt_obj >= start_of_window:
                        valid_items.append(item)
                    else:
                        reached_end_of_window = True
                        break
            else:
                # Backfill: keep everything
                valid_items = items
