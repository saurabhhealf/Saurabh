...
PAGES_PER_INVOCATION = int(os.environ.get("PAGES_PER_INVOCATION", "100")) # Increased to 100
...
# ... (Helpers _utc_now_ts, _safe_json_loads, _get_secret_value same as messages.py) ...
# ... (Use the same helpers from messages.py above for these 3 functions) ...
...
# ... (DDB helpers same as messages.py above) ...
def _ddb_get(job_start_id: str) -> Optional[Dict[str, Any]]: return TABLE.get_item(Key={"job_start_id": job_start_id}).get("Item")
...
def _fetch_page(cursor: Optional[str]) -> Tuple[List[Dict[str, Any]], Optional[str]]:
    ...
    # Minimal params for tickets (NO page param)
    ...
    j = r.json()
    items = []
    if isinstance(j, dict): items = j.get("data") or []
